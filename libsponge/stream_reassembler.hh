...
struct block {
    ...
    size_t length = 0;
    std::string bytes{};
    // bool operator<(const block b) { return begin_index < b.begin_index; }
};
...
class StreamReassembler {
  private:
    ...
    std::set<block> m_unassembled_set{};
    size_t m_send_index = 0;
    size_t m_unassembled_byte = 0;
    bool m_eof = false;
    ByteStream _output;  //!< The reassembled in-order byte stream
    size_t _capacity;    //!< The maximum number of bytes

    int merge_block(block &blk1, const block &blk2);

  public:
    size_t head_index() const { return m_send_index; }
