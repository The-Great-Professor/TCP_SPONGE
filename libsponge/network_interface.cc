...
void NetworkInterface::send_datagram(const InternetDatagram &dgram, const Address &next_hop) {
    ...
    if (_table.count(next_hop_ip) && _timer <= _table[next_hop_ip].ttl) {
        ...
        _pending_ip.push(next_hop_ip);
        retransmission_arp_frame();
...
optional<InternetDatagram> NetworkInterface::recv_frame(const EthernetFrame &frame) {
    if (!ethernet_address_equal(frame.header().dst, _ethernet_address) &&
        !ethernet_address_equal(frame.header().dst, ETHERNET_BROADCAST)) {
        return nullopt;
    }
    ...
    if (frame.header().type == EthernetHeader::TYPE_ARP) {
        ...
        if (msg.parse(frame.payload()) == ParseResult::NoError) {
            ...
            if (msg.opcode == ARPMessage::OPCODE_REQUEST && msg.target_ip_address == _ip_address.ipv4_numeric()) {
                ...
                out_frame.payload() = move(reply.serialize());
                _frames_out.push(out_frame);
            }

            while (!_pending_ip.empty()) {
                uint32_t p_ip = _pending_ip.front();
                if (_table.count(p_ip) && _timer <= _table[p_ip].ttl) {
                    _pending_ip.pop();
                    _pending_flag = false;
                } else
                    break;
...
void NetworkInterface::retransmission_arp_frame() {
    if (!_pending_ip.empty()) {
        if (!_pending_flag || (_pending_flag && _timer - _pending_timer >= 5000)) {
            uint32_t ip = _pending_ip.front();
