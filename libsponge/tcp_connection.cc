...
void TCPConnection::segment_received(const TCPSegment &seg) {
    ...
    // if (seg.header().rst) {  // rst
    //     _receiver.stream_out().set_error();
    //     _sender.stream_in().set_error();
    //     _active = false;
    // } else if (_sender.next_seqno_absolute() == 0) {  // close/listen
    //     if (seg.header().syn) {
    //         _receiver.segment_received(seg);
    //         connect();
    //     }
    // } else if (_sender.next_seqno_absolute() == _sender.bytes_in_flight() &&
    //            !_receiver.ackno().has_value()) {  // syn-sent
    ...
    //            !_receiver.stream_out().input_ended()) {  // syn-recv
    //     _sender.ack_received(seg.header().ackno, seg.header().win);
    //     _receiver.segment_received(seg);
    // } else if (_sender.next_seqno_absolute() > _sender.bytes_in_flight() &&
    //            !_sender.stream_in().eof()) {  // established
    //     _sender.ack_received(seg.header().ackno, seg.header().win);
    //     _receiver.segment_received(seg);
    ...
    // &&
    //            _sender.bytes_in_flight() > 0 && !_receiver.stream_out().input_ended()) {  // fin-wait1
    //     if (seg.header().fin) {
    //         _sender.ack_received(seg.header().ackno, seg.header().win);
    //         _receiver.segment_received(seg);
    //         _sender.send_empty_segment();
    ...
    // } else if (_sender.stream_in().eof() && _sender.next_seqno_absolute() == _sender.stream_in().bytes_written() + 2
    // &&
    //            _sender.bytes_in_flight() == 0 && !_receiver.stream_out().input_ended()) {  // fin-wait2
    //     _sender.ack_received(seg.header().ackno, seg.header().win);
    //     _receiver.segment_received(seg);
    //     _sender.send_empty_segment();
    //     send_segment();
    // } else if (_sender.stream_in().eof() && _sender.next_seqno_absolute() == _sender.stream_in().bytes_written() + 2
    // &&
    //            _sender.bytes_in_flight() == 0 && _receiver.stream_out().input_ended()) {  // time-wait
    //     if (seg.header().fin) {
    //         _sender.ack_received(seg.header().ackno, seg.header().win);
    //         _receiver.segment_received(seg);
    //         _sender.send_empty_segment();
...
//         else if (_sender.bytes_in_flight() == 0 && _sender.stream_in().eof()) {
